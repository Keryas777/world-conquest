...
static class PairLocalFrontierLab
{
    ...
    const double ConnectionToleranceDegrees = 1e-7;
    ...
    public static async Task GenerateAsync(string outDir)
    {
        ...
        var edges = root.GetProperty("edges").EnumerateArray()
            .Select(e => new Edge(e.GetProperty("a").GetInt64(), e.GetProperty("b").GetInt64(), e.TryGetProperty("foreign", out var foreign) && foreign.GetBoolean()))
            .ToArray();
        ...
        var pairMetrics = new List<PairMetric>();
        ...
        foreach (var pairKey in pairKeys)
        {
            ...
            var pairCells = targetCells
                .Where(c => c.OwnerCode.Equals(ownerA, StringComparison.OrdinalIgnoreCase) || c.OwnerCode.Equals(ownerB, StringComparison.OrdinalIgnoreCase))
            ...
            Geometry AssignedTo(string owner) => SafeUnion(pairCells
                .Where(c => c.OwnerCode.Equals(owner, StringComparison.OrdinalIgnoreCase))
                .Select(c => pairVoronoi.TryGetValue(c.Id, out var face) ? SafeIntersection(face, pairDomain) : GeometryFactory.CreatePolygon()));

            // Desired Voronoi result is first canonicalized into an exact two-owner partition.
            var desiredA = SafeIntersection(AssignedTo(ownerA), pairDomain);
            var desiredB = SafeDifference(pairDomain, desiredA);

            // Start from the current partition inside this exclusive pair domain. A transfer
            // is allowed only when the gained polygon is physically attached to territory the
            // gaining owner already controls. Detached Voronoi islands simply stay with their
            // current owner instead of forcing the whole pair update to be rejected.
            var currentAIn = SafeIntersection(ownerRegions[ownerA], pairDomain);
            var currentBIn = SafeDifference(pairDomain, currentAIn);
            var gainA = SafeDifference(desiredA, currentAIn);
            var gainB = SafeDifference(desiredB, currentBIn);
            var acceptedGainA = KeepConnectedGain(gainA, currentAIn);
            var acceptedGainB = KeepConnectedGain(gainB, currentBIn);
            var detachedGainA = SafeDifference(gainA, acceptedGainA);
            var detachedGainB = SafeDifference(gainB, acceptedGainB);

            var insideA = SafeUnion(new[] { SafeDifference(currentAIn, acceptedGainB), acceptedGainA });
            insideA = SafeIntersection(insideA, pairDomain);
            ...
            {
                Console.WriteLine($"C3 pair {pairKey}: topology guard still rejected connected-gain update ({ownerA} {candidateComponentsA}/{baselineComponentsA}, {ownerB} {candidateComponentsB}/{baselineComponentsB} components).");
            }

            pairMetrics.Add(new PairMetric(
                pairKey,
                widthKm,
                influenceKm,
                pairCells.Length,
                exclusiveCorridor.Area,
                localGap.Area,
                localOverlap.Area,
                detachedGainA.Area,
                detachedGainB.Area,
                topologyRejected,
                candidateComponentsA,
                candidateComponentsB,
                ownerA,
                ownerB));
        }
        ...
        var topology = ownerRegions.OrderBy(x => x.Key).Select(x =>
        {
            ...
            return new { ownerCode = x.Key, baselineComponents = baselineCount, finalComponents = finalCount, newComponents = Math.Max(0, finalCount - baselineCount) };
        }).ToArray();
        ...
        {
            status = "experimental",
            description = "C3 pair-local frontier experiment. Each terrestrial owner-pair has an exclusive corridor; only nearby cities of that pair compete. Voronoi gains are accepted only when attached to territory already controlled by the gaining owner, while the topology guard remains a final safety net.",
            ...
            c3 = new
            {
                ...
                pairMetrics,
                invariants = new { satisfied = invariantsSatisfied, coverageSatisfied, overlapSatisfied, topologySatisfied, gapArea = gap.Area, outsideArea = outside.Area, overlapArea, topology }
            },
        ...
        Console.WriteLine($"C3 pair-local: pairs={pairKeys.Length}, gap={gap.Area:F8}, overlap={overlapArea:F8}, new-components={topology.Sum(x => x.newComponents)}, rejected-pairs={pairMetrics.Count(x => x.TopologyRejected)}, detached-gain={pairMetrics.Sum(x => x.DetachedGainAreaA + x.DetachedGainAreaB):F8}, invariants={(invariantsSatisfied ? "ok" : "FAILED")}.");
        if (!invariantsSatisfied)
            throw new InvalidOperationException($"C3 geometry invariants failed: coverage={coverageSatisfied}, overlap={overlapSatisfied}, topology={topologySatisfied}.");
    }

    sealed record PairMetric(
        string Pair,
        double WidthKm,
        double InfluenceKm,
        int CandidateSiteCount,
        double CorridorArea,
        double LocalGapArea,
        double LocalOverlapArea,
        double DetachedGainAreaA,
        double DetachedGainAreaB,
        bool TopologyRejected,
        int CandidateComponentsA,
        int CandidateComponentsB,
        string OwnerA,
        string OwnerB);

    static Geometry KeepConnectedGain(Geometry gain, Geometry anchor)
    {
        if (gain.IsEmpty || anchor.IsEmpty) return GeometryFactory.CreatePolygon();
        var nearAnchor = SafeBuffer(anchor, ConnectionToleranceDegrees);
        return SafeUnion(PolygonParts(gain).Where(part => !SafeIntersection(part, nearAnchor).IsEmpty));
    }

    static IEnumerable<Polygon> PolygonParts(Geometry geometry)
    {
        ...
        for (var i = 0; i < geometry.NumGeometries; i++)
            foreach (var part in PolygonParts(geometry.GetGeometryN(i)))
                yield return part;
    }
    ...
    static Dictionary<long, Geometry> BuildVoronoi(IReadOnlyList<Cell> cells, Envelope envelope)
    {
        ...
        foreach (var grid in snapGrids)
        {
            ...
            {
                var point = grid <= 0 ? new Coordinate(cell.Lon, cell.Lat) : new Coordinate(Snap(cell.Lon, grid), Snap(cell.Lat, grid));
                if (representatives.TryAdd(Key(point), cell)) snappedById[cell.Id] = point;
            }
            ...
            try
            {
                var builder = new VoronoiDiagramBuilder { ClipEnvelope = envelope, Tolerance = grid <= 0 ? 0.0 : grid };
    ...
    static Dictionary<long, Geometry> BuildVoronoiByHalfPlanes(IReadOnlyList<Cell> cells, Envelope envelope)
    {
        ...
        var clip = new Envelope(envelope);
        clip.ExpandBy(margin);
        var result = new Dictionary<long, Geometry>();
        foreach (var site in cells.OrderBy(c => c.Id))
        {
            ...
            foreach (var other in cells.OrderBy(c => c.Id))
            {
                ...
                var dx = other.Lon - site.Lon;
                var dy = other.Lat - site.Lat;
                if (Math.Abs(dx) < 1e-12 && Math.Abs(dy) < 1e-12) continue;
                var mx = (site.Lon + other.Lon) * 0.5;
                var my = (site.Lat + other.Lat) * 0.5;
                polygon = ClipHalfPlane(polygon, dx, dy, dx * mx + dy * my);
                if (polygon.Count < 3) break;
            }
            if (polygon.Count >= 3)
            {
                var ring = polygon.Select(p => new Coordinate(p)).ToList();
                ring.Add(new Coordinate(ring[0]));
    ...
    static List<Coordinate> ClipHalfPlane(IReadOnlyList<Coordinate> input, double a, double b, double c)
    {
        ...
        Coordinate Intersect(Coordinate p, Coordinate q)
        {
            var vx = q.X - p.X;
            var vy = q.Y - p.Y;
            var denom = a * vx + b * vy;
            if (Math.Abs(denom) < 1e-15) return new Coordinate(p);
            var t = (c - a * p.X - b * p.Y) / denom;
            return new Coordinate(p.X + t * vx, p.Y + t * vy);
        }
        ...
        foreach (var current in input)
        {
            ...
            if (currentInside)
            {
                ...
                output.Add(new Coordinate(current));
            }
            else if (previousInside) output.Add(Intersect(previous, current));
            previous = current;
    ...
    static Cell ParseCell(JsonElement e)
    {
        ...
        return new Cell(e.GetProperty("id").GetInt64(), e.GetProperty("territoryCode").GetString() ?? "?", e.GetProperty("ownerCode").GetString() ?? "?", e.GetProperty("lat").GetDouble(), e.GetProperty("lon").GetDouble(), geometry);
    }
    ...
    static Geometry SafeUnion(IEnumerable<Geometry> geometries)
    {
        ...
        try { var r = UnaryUnionOp.Union(values); return r.IsValid ? r : r.Buffer(0); }
        catch { var r = OverlayNGRobust.Union(values); return r.IsValid ? r : r.Buffer(0); }
    }
    ...
    static Geometry SafeIntersection(Geometry a, Geometry b)
    {
        ...
        catch { var r = OverlayNGRobust.Overlay(a, b, SpatialFunction.Intersection); return r.IsValid ? r : r.Buffer(0); }
    }
    ...
    static Geometry SafeDifference(Geometry a, Geometry b)
    {
        ...
        catch { var r = OverlayNGRobust.Overlay(a, b, SpatialFunction.Difference); return r.IsValid ? r : r.Buffer(0); }
    }

    static Geometry SafeBuffer(Geometry geometry, double distance)
    {
        try { var r = geometry.Buffer(distance); return r.IsValid ? r : r.Buffer(0); }
        catch { var r = geometry.Buffer(0).Buffer(distance); return r.IsValid ? r : r.Buffer(0); }
    }
