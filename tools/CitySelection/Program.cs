...
static class Geo
{
    public static double Km(City a, City b)
    {
        ...
        var dLat = Rad(b.Lat - a.Lat); var dLon = Rad(b.Lon - a.Lon); var la1 = Rad(a.Lat); var la2 = Rad(b.Lat);
...
static class Selector
{
    public static List<City> Select(IReadOnlyList<City> candidates, int quota, double populationWeight)
    {
        ...
        var remaining = candidates.OrderByDescending(c => c.Population).ToList(); var selected = new List<City> { remaining[0] }; remaining.RemoveAt(0);
        var nearest = remaining.ToDictionary(c => c.Id, c => Geo.Km(c, selected[0]));
        while (selected.Count < quota && remaining.Count > 0)
        {
            var maxNearest = Math.Max(1.0, remaining.Max(c => nearest[c.Id])); City? best = null; var bestScore = double.NegativeInfinity;
            foreach (var c in remaining)
            {
                var score = populationWeight * Pop(c) + (1 - populationWeight) * Math.Min(1.0, nearest[c.Id] / maxNearest);
                if (score > bestScore || (Math.Abs(score - bestScore) < 1e-12 && c.Population > (best?.Population ?? -1))) { best = c; bestScore = score; }
            }
            if (best is null) break; selected.Add(best); remaining.Remove(best); nearest.Remove(best.Id);
            foreach (var c in remaining) nearest[c.Id] = Math.Min(nearest[c.Id], Geo.Km(c, best));
        }
        return selected;
    }
}
...
static class Results
{
    public static (double Mean, double Min, double Max) NearestStats(IReadOnlyList<City> cities)
    {
        ...
        for (var i = 0; i < cities.Count; i++) { var d = double.PositiveInfinity; for (var j = 0; j < cities.Count; j++) if (i != j) d = Math.Min(d, Geo.Km(cities[i], cities[j])); if (!double.IsInfinity(d)) values.Add(d); }
    ...
    {
        Directory.CreateDirectory(Path.GetDirectoryName(path)!); using var w = new StreamWriter(path); w.WriteLine("geoname_id,name,country,latitude,longitude,population");
        foreach (var c in cities.OrderBy(c => c.Country).ThenByDescending(c => c.Population)) w.WriteLine($"{c.Id},\"{c.Name.Replace("\"", "\"\"")}\",{c.Country},{c.Lat.ToString(CultureInfo.InvariantCulture)},{c.Lon.ToString(CultureInfo.InvariantCulture)},{c.Population}");
    }
    public static void WriteSvg(string path, IReadOnlyDictionary<string, List<City>> groups, double weight)
    {
        var all = groups.Values.SelectMany(x => x).ToList(); var minLon = all.Min(c => c.Lon); var maxLon = all.Max(c => c.Lon); var minLat = all.Min(c => c.Lat); var maxLat = all.Max(c => c.Lat); const int width = 1000, height = 900, pad = 45;
        double X(double lon) => pad + (lon - minLon) / Math.Max(.001, maxLon - minLon) * (width - 2 * pad); double Y(double lat) => height - pad - (lat - minLat) / Math.Max(.001, maxLat - minLat) * (height - 2 * pad);
        var colors = new Dictionary<string, string> { ["FR"] = "#3b82f6", ["BE"] = "#f59e0b", ["LU"] = "#ef4444" }; using var w = new StreamWriter(path); w.WriteLine($"<svg xmlns=\"http://www.w3.org/2000/svg\" width=\"{width}\" height=\"{height}\" viewBox=\"0 0 {width} {height}\">"); w.WriteLine("<rect width=\"100%\" height=\"100%\" fill=\"#f7f7f7\"/>"); w.WriteLine($"<text x=\"45\" y=\"30\" font-family=\"sans-serif\" font-size=\"20\">City selection — population weight {weight:P0}</text>");
        foreach (var (country, cities) in groups) foreach (var c in cities) { var title = System.Security.SecurityElement.Escape($"{c.Name} — {c.Population:N0}"); w.WriteLine($"<circle cx=\"{X(c.Lon):F1}\" cy=\"{Y(c.Lat):F1}\" r=\"4\" fill=\"{colors[country]}\" fill-opacity=\".75\"><title>{title}</title></circle>"); }
        w.WriteLine("</svg>");
    }
}
    ...
    public static async Task Main(string[] args)
    {
        ...
        if (args.Contains("--frontier-only", StringComparer.OrdinalIgnoreCase))
        {
            ...
            if (!File.Exists(publishedGraph))
                throw new FileNotFoundException("Frontier-only mode requires docs/voronoi-graph.json.", publishedGraph);
        ...
        var cacheDir = Path.Combine("data", "raw", "geonames"); Directory.CreateDirectory(cacheDir);
